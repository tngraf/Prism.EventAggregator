...
// ===========================================================================
//
// <copyright file="PubSubEvent.cs" company="Tethys">
...
namespace Prism.EventAggregator
{
    ...
    /// </summary>
    /// <typeparam name="TPayload">The type of message that will be passed to the 
    ...
    public class PubSubEvent<TPayload> : EventBase
    {
        ...
        /// <see cref="PubSubEvent{TPayload}"/> will maintain a <see cref="WeakReference"/> 
        ...
        /// the <see cref="PubSubEvent{TPayload}"/> keeps a reference to the subscriber
        ///  so it does not get garbage collected.</param>
        /// <returns>A <see cref="SubscriptionToken"/> that uniquely identifies the 
        /// added subscription.</returns>
        /// <remarks>
        /// If <paramref name="keepSubscriberReferenceAlive"/> is set to <see langword="false" />, 
        /// <see cref="PubSubEvent{TPayload}"/> will maintain a <see cref="WeakReference"/> to
        /// the Target of the supplied <paramref name="action"/> delegate.
        /// If not using a WeakReference (<paramref name="keepSubscriberReferenceAlive"/> 
        /// is <see langword="true" />), the user must explicitly call Unsubscribe for the 
        /// event when disposing the subscriber in order to avoid memory leaks or unexpected 
        /// behavior.
        /// <para/>
        /// The PubSubEvent collection is thread-safe.
        /// </remarks>
        ...
        public SubscriptionToken Subscribe(Action<TPayload> action, 
            ...
            bool keepSubscriberReferenceAlive)
        {
            return this.Subscribe(action, threadOption, 
                keepSubscriberReferenceAlive, null);
        } // Subscribe()
        ...
        /// <param name="action">The delegate that gets executed when the event is 
        /// published.</param>
        /// <param name="threadOption">Specifies on which thread to receive the delegate
        /// callback.</param>
        /// <param name="keepSubscriberReferenceAlive">When <see langword="true"/>, the
        /// <see cref="PubSubEvent{TPayload}"/> keeps a reference to the subscriber so it
        /// does not get garbage collected.</param>
        /// <param name="filter">Filter to evaluate if the subscriber should receive the
        /// event.</param>
        /// <returns>A <see cref="SubscriptionToken"/> that uniquely identifies the added
            ...
            Predicate<TPayload> filter)
        {
            ...
            if (filter != null)
            {
                filterReference = new DelegateReference(filter, 
                    keepSubscriberReferenceAlive);
            }
            else
            {
                filterReference = new DelegateReference(new Predicate<TPayload>(
                    delegate { return true; }), true);
            } // if
            ...
            switch (threadOption)
            {
                ...
                case ThreadOption.BackgroundThread:
                    subscription = new BackgroundEventSubscription<TPayload>(
                        actionReference, filterReference);
                    break;
                case ThreadOption.UIThread:
                    ...
                    subscription = new DispatcherEventSubscription<TPayload>(
                        actionReference, filterReference, this.SynchronizationContext);
                    break;
                default:
                    subscription = new EventSubscription<TPayload>(actionReference,
                        filterReference);
        ...
        /// <summary>
        /// Removes the first subscriber matching <see cref="Action{TPayload}"/>
        /// from the subscribers' list.
        /// </summary>
        /// <param name="subscriber">The <see cref="Action{TPayload}"/> used when
        /// subscribing to the event.</param>
        public virtual void Unsubscribe(Action<TPayload> subscriber)
        {
            lock (this.Subscriptions)
            {
                IEventSubscription eventSubscription = this.Subscriptions
                    .Cast<EventSubscription<TPayload>>().FirstOrDefault(
        ...
        /// <see cref="Action{TPayload}"/>.
        /// </summary>
        /// <param name="subscriber">The <see cref="Action{TPayload}"/> used when
        /// subscribing to the event.</param>
        /// <returns><see langword="true"/> if there is an <see cref="Action{TPayload}"/>
        ...
        public virtual bool Contains(Action<TPayload> subscriber)
        {
            ...
            lock (this.Subscriptions)
            {
                eventSubscription = this.Subscriptions.Cast<EventSubscription<TPayload>>()
